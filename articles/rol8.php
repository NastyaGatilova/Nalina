...
<section id="sush1">
	<p>	
		<?php  $result = mysqli_query($db, 'SELECT * FROM `dish` WHERE iddish = 12');
		$row = mysqli_fetch_array($result, MYSQLI_ASSOC);?>
		<?php foreach ($result as $row){ ?>
		<h4 align="center"><?php echo  $row['name']; 
					} ?></h4>

				<?php  	$result = mysqli_query($db, 'SELECT * FROM `picture` WHERE page_idpage = 38');
$row = mysqli_fetch_array($result, MYSQLI_ASSOC);?>
		<?php foreach ($result as $row){ ?>
		<img src="<?php echo $row['file_path'] ?>" width="350" height="350" 
	  	...
	  	<?php } ?>	
	  	<?php
	  	$result = mysqli_query($db, 'SELECT * FROM `dish` WHERE iddish = 12');
		$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
		...
		<div align = "left"> <a href = "index.php?page=update&iddish=12&category_idcategory=2" >Редактировать</a></div>
		</section>