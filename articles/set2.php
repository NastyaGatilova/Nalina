...
<section id="sush1">
	<p>	
			<?php  $result = mysqli_query($db, 'SELECT * FROM `dish` WHERE iddish = 14');
		$row = mysqli_fetch_array($result, MYSQLI_ASSOC);?>
...
$row = mysqli_fetch_array($result, MYSQLI_ASSOC);	 ?>
		...
		<img src="<?php echo $row['file_path'] ?>" width="350" height="350" 
	  	...
	  	<?php } ?>	
	  	<?php
	  	$result = mysqli_query($db, 'SELECT * FROM `dish` WHERE iddish = 14');
		$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
	...
	<div align = "left"> <a href = "index.php?page=update&iddish=14&category_idcategory=3" >Редактировать</a></div>
